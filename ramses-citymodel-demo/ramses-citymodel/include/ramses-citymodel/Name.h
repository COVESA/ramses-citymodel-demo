...
public:
    ...
    Name(const std::string& text, ramses::Scene& scene, ramses::RamsesClient& client);
     ...
     *  @param renderGroup RenderGroup to which the geometry is added. */
    void createGeometry(ramses::TextCache& textCache, ramses::FontInstanceId fontInstance, ramses::Effect& effect, ramses::RenderGroup& renderGroup);

    /// Returns the minimum bounding coordinates of the text label geometry.
    /** @return The minimum bounding. */
    const Vector2& minBounding() const;

    /// Returns the maximum bounding coordinates of the text label geometry.
    /** @return The maximum bounding. */
    const Vector2& maxBounding() const;
