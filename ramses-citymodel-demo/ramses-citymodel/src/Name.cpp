...
void Name::createGeometry(ramses::TextCache& textCache, ramses::FontInstanceId fontInstance, ramses::Effect& effect, ramses::RenderGroup& renderGroup)
{
    ...
    const float dx = -floor(x + w * 0.5f);
    const float dy = -floor(y + h * 0.5f);

    m_minBounding = Vector2(x, y);
    m_maxBounding = Vector2(x + w, y + h);
    ...
    m_textLocalTranslate->setTranslation(dx, dy, 0.0f);
...
const Vector2& Name::minBounding() const
{
    return m_minBounding;
}

const Vector2& Name::maxBounding() const
{
