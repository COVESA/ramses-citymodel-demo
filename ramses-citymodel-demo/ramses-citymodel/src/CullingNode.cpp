...
{
    return a->center().getX() < b->center().getX();
}

bool lessY(Tile* a, Tile* b)
{
    return a->center().getY() < b->center().getY();
}
...
void CullingNode::splitX(const SortedTileSet& tiles, SortedTileSet& tilesLower, SortedTileSet& tilesUpper)
{
    ...
    float median = tiles.m_tilesX[medianIndex(static_cast<uint32_t>(tiles.m_tilesX.size()))]->center().getX();

    for (uint32_t i = 0; i < tiles.m_tilesX.size(); i++)
    {
        if (tiles.m_tilesX[i]->center().getX() <= median)
        {
    ...
    {
        if (tiles.m_tilesY[i]->center().getX() <= median)
        {
...
void CullingNode::splitY(const SortedTileSet& tiles, SortedTileSet& tilesLower, SortedTileSet& tilesUpper)
{
    ...
    float median = tiles.m_tilesY[medianIndex(static_cast<uint32_t>(tiles.m_tilesY.size()))]->center().getY();
    for (uint32_t i = 0; i < tiles.m_tilesX.size(); i++)
    {
        if (tiles.m_tilesX[i]->center().getY() <= median)
        {
            tilesLower.m_tilesX.push_back(tiles.m_tilesX[i]);
        }
        else
    ...
    for (uint32_t i = 0; i < tiles.m_tilesY.size(); i++)
    {
        if (tiles.m_tilesY[i]->center().getY() <= median)
        {
            tilesLower.m_tilesY.push_back(tiles.m_tilesY[i]);
        }
        else
